function Get-SecureSphereAssessmentScanRunsRecords {
    ...
    Begin {
        ...
        $ApiUrl = "$SecureSphereHost/SecureSphere/api/v1/conf/dasAssessment/scanRecords"

        $ApiBody = [ordered]@{}
        if ($PSBoundParameters.ContainsKey('RunIds')) {
            $ApiBody += @{ "run-ids" = $RunIds }
        }
        if ($PSBoundParameters.ContainsKey('ResultIds')) {
            $ApiBody += @{ "result-ids" = $ResultIds }
        }
